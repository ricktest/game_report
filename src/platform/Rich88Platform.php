...
class Rich88Platform implements PlatformInterface
{
    ...
    {
        // 實現返回字串的邏輯
        return "SUM(CASE WHEN `action` = 'withdraw' THEN money ELSE 0 END)";
    }

    public function getSumWinOrder(): string
    {
        // 實現返回字串的邏輯
        return "SUM(CASE WHEN `action` = 'deposit' THEN money ELSE 0 END)- SUM(CASE WHEN `action` = 'withdraw' THEN money ELSE 0 END)";
    }

    public function getExcludingCanceled(): string
    {
        // 實現返回字串的邏輯
        return "transaction_status IS null";
    }
